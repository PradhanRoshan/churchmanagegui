...
export class ApplicationDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const fromNavigation = (history.state as any)?.applicationDetails;
    if (fromNavigation) {
  ...
  goBack(): void {
    if(this.applicationDetails.applicationStatus?.statusName === 'Rejected' || this.applicationDetails.userMember?.applicationSts === 'Rejected') {
      this.router.navigate(['/dashboard/applications'], { fragment: 'rejected' });
  ...
  getCommentTimestampClass(comment: RegistrationComment): string {
    void comment;
    return 'text-muted';
  }
